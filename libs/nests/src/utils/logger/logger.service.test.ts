...
describe("Logger", () => {
	describe("[static methods]", () => {
		describe("when the default logger is used", () => {
			...
			it("should print multiple messages to the console", () => {
				...
				assertStringIncludes(
					getStringFromBytes(
						processStdoutWriteSpy.secondCall.firstArg,
					),
					`[${context}]`,
				);
				assertStringIncludes(
				...
				assertStringIncludes(
					getStringFromBytes(
						processStdoutWriteSpy.thirdCall.firstArg,
					),
					`[${context}]`,
				);
				assertStringIncludes(
			...
			it("should print one error to the console with stacktrace", () => {
				...
				assertEquals(
					getStringFromBytes(
						processStderrWriteSpy.secondCall.firstArg,
					),
					stacktrace + "\n",
				);
			});
			...
			it("should print multiple 2 errors and one stacktrace to the console", () => {
				...
				assertEquals(
					getStringFromBytes(
						processStderrWriteSpy.thirdCall.firstArg,
					),
					stack + "\n",
				);
			});
		...
		describe("when logging is disabled", () => {
			...
			beforeEach(() => {
				...
				previousLoggerRef = Logger["localInstanceRef"] ||
		...
		describe("when custom logger is being used", () => {
			...
			it('should call custom logger "#log()" method', () => {
				...
				assertEquals(
					customLoggerLogSpy.calledWith(message, context),
					true,
				);
			});
			...
			it('should call custom logger "#error()" method', () => {
				...
				assertEquals(
					customLoggerErrorSpy.calledWith(message, context),
					true,
				);
			});
		});
		...
		describe("when custom logger is being used", () => {
			...
			describe("with global context", () => {
				...
				it('should call custom logger "#log()" method with context as second argument', () => {
					...
					assertEquals(customLoggerLogSpy.called, true);
					assertEquals(
						customLoggerLogSpy.calledWith(message, globalContext),
						true,
					);
				});
				it('should call custom logger "#error()" method with context as third argument', () => {
					...
					const customLoggerErrorSpy = sinon.spy(
						customLogger,
						"error",
					);
					...
					assertEquals(customLoggerErrorSpy.called, true);
					assertEquals(
						customLoggerErrorSpy.calledWith(
							message,
							undefined,
							globalContext,
						),
	...
	describe("ConsoleLogger", () => {
		...
		it.only("should stringify messages", () => {
			...
			assertEquals(
				consoleLoggerSpy.getCall(1).returnValue,
				`Object:
{
  "key": "str2"
}
`,
			...
			assertEquals(
				consoleLoggerSpy.getCall(2).returnValue,
				`Object:
[
  "str3"
]
`,
			...
			assertEquals(
				consoleLoggerSpy.getCall(3).returnValue,
				`Object:
[
  {
    "key": "str4"
